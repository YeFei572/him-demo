...
import com.lmxdawn.him.api.ss.SocketServerInitializer;
...
public class WSServer {
    ...
    private EventLoopGroup socketBoss = new NioEventLoopGroup();
    private EventLoopGroup socketWork = new NioEventLoopGroup();
    ...
    private void startSocketServer() throws InterruptedException {
        ServerBootstrap bootstrap = new ServerBootstrap()
                .group(socketBoss, socketWork)
                .channel(NioServerSocketChannel.class)
                .localAddress(new InetSocketAddress(ssPort))
                .childOption(ChannelOption.SO_KEEPALIVE, true)
                .childHandler(new SocketServerInitializer());
        ChannelFuture future = bootstrap.bind().sync();
        if (future.isSuccess()) {
            log.info("启动socket server成功！");
        }
    }
    ...
    public void destroy() {
        ...
        socketBoss.shutdownGracefully().syncUninterruptibly();
        socketWork.shutdownGracefully().syncUninterruptibly();
        log.info("关闭 ws server 成功");
        log.info("关闭 socket server 成功");
    }
